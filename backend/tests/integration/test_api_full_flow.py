...
class TestAPIFullFlow:
    ...
    def test_api_full_flow_create_to_export(self, project_id):
        """
        Test complete API flow: Create project → Upload template → Outline → Descriptions → Images (with template) → Export ZIP
        ...
        response = requests.post(
            ...
            json={
                'creation_type': 'idea',
                'idea_prompt': '创建一套关于人工智能基础的电商图片图集，包含3张：什么是AI、AI的应用、AI的未来'
            },
            timeout=30
        )
        ...
        # Step 6: Export images ZIP
        print('📦 Step 6: Exporting images ZIP...')
        response = requests.get(
            f"{BASE_URL}/api/projects/{pid}/export/images?filename=integration-test.zip",
            timeout=60
        )
        ...
        assert data['data']['download_url'].endswith('.zip')
        
        print(f"  Export URL: {data['data']['download_url']}")
        
        # Step 7: Verify ZIP can be downloaded
        print('📥 Step 7: Verifying ZIP file can be downloaded...')
        ...
        assert len(zip_bytes) > 200  # ZIP should not be empty

        with zipfile.ZipFile(io.BytesIO(zip_bytes), 'r') as zf:
            names = zf.namelist()
            assert any(name.lower().endswith('.jpg') for name in names), f"Expected JPGs in ZIP, got: {names}"

        print(f"✓ ZIP downloaded successfully, size: {len(zip_bytes) / 1024:.2f} KB\n")
