...
def create_project():
    ...
    try:
        ...
        if project_type != "ecom":
            return bad_request("Invalid project_type")

        page_aspect_ratio = (data.get("page_aspect_ratio") or "").strip() or "3:4"
        cover_aspect_ratio = (data.get("cover_aspect_ratio") or "").strip() or "1:1"

        image_model = data.get("image_model")
        if image_model is not None:
            image_model = str(image_model).strip() or None
...
def update_project(project_id):
    ...
    try:
        ...
        # Update project-level image model override if provided
        if 'image_model' in data:
            value = data.get('image_model')
            project.image_model = (str(value).strip() if value is not None else '') or None

        # Update aspect ratios if provided
        if 'page_aspect_ratio' in data:
            value = (str(data.get('page_aspect_ratio') or '')).strip()
            project.page_aspect_ratio = value or '3:4'
        if 'cover_aspect_ratio' in data:
            value = (str(data.get('cover_aspect_ratio') or '')).strip()
            project.cover_aspect_ratio = value or '1:1'

        if 'project_type' in data:
            value = (str(data.get('project_type') or '')).strip().lower()
            if value and value != 'ecom':
                return bad_request("Invalid project_type")
            if value:
