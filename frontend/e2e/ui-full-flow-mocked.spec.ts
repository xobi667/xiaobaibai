...
test.describe('UI-driven E2E test (Mocked Backend)', () => {
  ...
  test('User Full Flow: Create and export ZIP with mocked API', async ({ page }) => {
    console.log('\n========================================')
    ...
    // Mock images ZIP export
    await page.route('**/api/projects/*/export/images**', async (route) => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({
          success: true,
          data: {
            download_url: '/files/mock-project-123/exports/mock-images.zip'
          }
        })
      })
    })
    ...
    if (await exportBtn.count() > 0) {
      ...
      if (await exportZipBtn.count() > 0) {
        await exportZipBtn.first().click()
      }
      
      const download = await downloadPromise
      if (download) {
        const downloadPath = path.join('test-results', 'e2e-mocked-test-output.zip')
        await download.saveAs(downloadPath)
        console.log(`✓ Mock ZIP downloaded: ${downloadPath}\n`)
      } else {
