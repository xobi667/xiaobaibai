/**
 * UI-driven end-to-end test: From user interface operations to final images ZIP export
 * 
...
test.describe('UI-driven E2E test: From user interface to ZIP export', () => {
  // Increase timeout to 20 minutes
  test.setTimeout(20 * 60 * 1000)
  
  test('User Full Flow: Create and export images ZIP in browser', async ({ page }) => {
    console.log('\n========================================')
    ...
    // Verify page loaded
    await expect(page).toHaveTitle(/xobi/i)
    ...
    await ideaInput.fill('创建一套关于人工智能基础的电商图片图集，包含3张：什么是AI、AI的应用、AI的未来')
    ...
    if (await generateImageBtn.count() > 0) {
      ...
      try {
        ...
        const thumbnails = page.locator('[data-page-index], .page-thumbnail')
    ...
    // ====================================
    // Step 14: Export images ZIP
    // ====================================
    console.log('📦 Step 14: Exporting images ZIP...')
    ...
    // Step 2: Click "导出全部图片 ZIP" in the dropdown menu
    const exportZipBtn = page.locator('button:has-text("导出全部图片 ZIP")')
    await exportZipBtn.waitFor({ state: 'visible', timeout: 5000 })
    await exportZipBtn.click()
    console.log('✓ Clicked "导出全部图片 ZIP" button\n')
    ...
    // Save file
    const downloadPath = path.join('test-results', 'e2e-test-output.zip')
    ...
    // Quick validation: ZIP signature should start with "PK"
    const zipHeader = fs.readFileSync(downloadPath).subarray(0, 2).toString('utf-8')
