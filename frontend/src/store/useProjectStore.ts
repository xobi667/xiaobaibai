...
interface ProjectState {
  ...
  initializeProject: (
    ...
    projectOptions?: Partial<CreateProjectRequest>
  ) => Promise<void>;
...
const debouncedUpdatePage = debounce(
  ...
  initializeProject: async (type, content, templateImage, templateStyle, projectOptions) => {
    ...
    try {
      ...
      // 项目级可选配置（如生图模型/比例等）
      if (projectOptions) {
        if (projectOptions.project_type) request.project_type = projectOptions.project_type;
        if (projectOptions.page_aspect_ratio) request.page_aspect_ratio = projectOptions.page_aspect_ratio;
        if (projectOptions.cover_aspect_ratio) request.cover_aspect_ratio = projectOptions.cover_aspect_ratio;
        if (projectOptions.image_model) request.image_model = projectOptions.image_model;
      }
