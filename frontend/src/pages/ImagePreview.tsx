...
export const ImagePreview: React.FC = () => {
  ...
  // 同步当前项目的生图模型（项目级）
  useEffect(() => {
    const nextModel = currentProject?.image_model || 'gemini-3-pro-image-preview';
    setImageModel(nextModel);
  }, [currentProject?.id, currentProject?.image_model]);
  ...
  const handleImageModelChange = useCallback(
    async (value: string) => {
      ...
      try {
        await updateProject(projectId, { image_model: value });
        await syncProject(projectId);
        show({ message: '生图模型已更新', type: 'success' });
      } catch (error: any) {
        console.error('更新生图模型失败:', error);
        setImageModel(prev);
        const msg = error?.response?.data?.error?.message || error?.message || '未知错误';
        show({ message: `更新生图模型失败: ${normalizeErrorMessage(msg)}`, type: 'error' });
      } finally {
        setIsSavingImageModel(false);
      }
    },
    [projectId, imageModel, syncProject, show]
  );
          ...
          <div className="p-3 md:p-4 border-b border-gray-200 flex-shrink-0 space-y-2 md:space-y-3">
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
              ...
              <select
                ...
                disabled={isSavingImageModel}
                className="h-10 md:h-11 px-2 md:px-3 rounded-lg border border-gray-200 bg-white text-xs md:text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-banana-500 focus:border-transparent sm:max-w-[190px]"
                title="生图模型"
              >
