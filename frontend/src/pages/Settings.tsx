...
const settingsSections: SectionConfig[] = [
  {
    ...
    fields: [
      ...
      },
      {
        ...
        type: 'password',
        placeholder: '输入新的 Seedream API Key',
        sensitiveField: true,
        lengthKey: 'seedream_api_key_length',
        description: '用于 Seedream 4.5 生图模型（Base URL 与主 API 相同）；留空则保持当前设置不变，输入新值则更新',
      },
    ],
  },
  {
...
export const Settings: React.FC = () => {
  ...
  const handleSave = async () => {
    setIsSaving(true);
    try {
      const { api_key, seedream_api_key, mineru_token, ...otherData } = formData;
      ...
      if (seedream_api_key) {
        payload.seedream_api_key = seedream_api_key;
      }
      ...
      if (response.data) {
        ...
        setFormData(prev => ({ ...prev, api_key: '', seedream_api_key: '', mineru_token: '' }));
      }
    } catch (error: any) {
  ...
  const handleTestConnection = async () => {
    ...
    try {
      ...
      const rawMsg = (response.data?.message || '').toString().trim();
      const msg = rawMsg
        ? rawMsg
            .replace(/connection successful/gi, '连接成功')
            .replace(/test connection/gi, '测试连接')
            .replace(/successfully/gi, '成功')
        : '连接成功（仅测试文本/对话接口）！';
